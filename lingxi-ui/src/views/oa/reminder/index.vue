  ...
  <div class="app-container">
    <el-card>
      ...
      <div class="filter-bar">
        ...
        <el-select v-model="query.reminderType" placeholder="消息类型" size="mini" clearable @change="getList">
      ...
      <div class="message-list">
        ...
        >
          <div class="message-icon">
            <i v-if="item.reminderType === 'timeout'" class="el-icon-warning" style="color: #f56c6c;"></i>
            <i v-else-if="item.reminderType === 'task'" class="el-icon-s-claim" style="color: #409eff;"></i>
...
<script>
import { getReminderList, getUnreadReminderCount, markReminderAsRead } from '@/api/oa/ai'
...
export default {
  ...
  methods: {
    ...
    handleRead(item) {
      if (item.status === '0') {
        markReminderAsRead(item.reminderId).then(() => {
          item.status = '1'
    ...
    markAllRead() {
      this.$confirm('确定将所有消息标为已读吗？', '提示').then(() => {
        ...
        Promise.all(unreadItems.map(item => {
          return markReminderAsRead(item.reminderId).catch(() => {})
        })).then(() => {
